...
final class EditProfileViewController: UIViewController, UITableViewDataSource {
    ...
    private var models = [[EditProfileFormModal]]()
    
    
    override func viewDidLoad() {
        super.viewDidLoad()
        configureModels()
        table.tableHeaderView = createTableHeaderView()
    ...
    private func configureModels(){
        ...
        var section1 = [EditProfileFormModal]()
        for label in section1Labels {
            let models = EditProfileFormModal(label: label, placeholder: "Enter \(label)...", value: nil)
            section1.append(models)
        }
        
        models.append(section1)
        
        let section2Labels = ["Email","Phone","Gender"];
        var section2 = [EditProfileFormModal]()
        for label in section2Labels {
            let models = EditProfileFormModal(label: label, placeholder: "Enter \(label)...", value: nil)
            section2.append(models)
        }
...
extension EditProfileViewController: FormTableViewCellDelegate{
    func formTableViewCell(_ cell: FormTableViewCell, didUpdateField updatedModel: EditProfileFormModal?) {
